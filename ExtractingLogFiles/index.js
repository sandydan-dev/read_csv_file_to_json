...
function chooseLogMessage() {
    ...
    if (isSystemEvent) {
        return systemLogMessages[Math.floor(Math.random() * systemLogMessages.length)];
    }
...
function randomTimestamp() {
    ...
    return new Date(randomPastTime).toISOString().replace('T', ' ').replace('Z', '');
}
